...
- Major roots are `agilex_ws/` for real robot packages (standard catkin: `src/` + `scripts/`), `ocs2_ws/` for OCS2, `remani_planner/` for REMANI-Planner, `robotics-toolbox-python/` for Holistic/RTB experiments, and root `scripts/` for local adaptation scripts.
- `agilex_ws/` is a standard catkin workspace: packages live under `agilex_ws/src/`; host helpers under `agilex_ws/scripts/`. Builds use `catkin_make -C agilex_ws --source agilex_ws/src` (or plain `catkin_make` from `agilex_ws` after `src` exists). The former flat `agx/` source-root layout is retired.
...
- Real-mode laptop deployment is control-plane only: planner/Gate/State Bridge/dry-run Executor/RViz run on the notebook; `agilex_ws/` on the robot is a remote black-box ROS endpoint. Do not compile, source, or start `agilex_ws/` packages on the laptop for Phase 2 dry-run work.
- Ranger + CR10 hardware attach to an Ubuntu 18.04 x86-64 host where `agilex_ws/` previously built and ran. Laptop development hosts (e.g. Ubuntu 20.04/Noetic) may lack standalone `asio.hpp` and must not treat a failed local `ugv_sdk` build as an AGX driver regression; full `ranger_base`/`ugv_sdk`/`dobot_v4_bringup` verification belongs on that 18.04 machine.
...
- Physical split: robot host workspace is `agilex_ws/` (`scripts/` + `src/`); laptop owns `remani_planner/scripts/` control-plane helpers. Robot `run_real_dry_run.sh` launches drivers only; `run_lidar.sh` / `run_camera.sh` start RoboSense `/rslidar_points` and RealSense `/camera/...` (build via `build_agx_sensors.sh`). Laptop `run_real_dry_run_control_plane.sh` runs State Bridge/Gate/Executor/planner/RViz with `start_hardware_drivers:=false` and `use_fake_feedback:=false` on a shared `ROS_MASTER_URI`. V1 `static_empty` does not consume lidar/camera for planning.
- `/remani/cr10_status` is not published by AGX drivers (they use `/dobot_v4_bringup/msg/RobotStatus`). Until a laptop-side converter exists, real-feedback dry-run needs an explicit `Cr10Status` publisher or readiness stays NotReady.
