...
- Bridge/arch: `generateBridge` (two pillars + lintel). Default clearance (lintel bottom) **1.5 m**; opening width **~2.8–3.5 m** so tall pillars clear the chassis/arm laterally. CR10 home arm reaches ~1.57 m and **hits** a 1.5 m lintel (box under chassis still clears). `obs_num[2]` = bridges (default 0); `bridge_enable` must be true. Ordinary smoke stays bridge-free; dedicated `map_ranger_cr10_bridge.yaml` + `run_ranger_cr10_bridge.launch`. Gate: `test_bridge_obstacle_clearance`.
- Plan stage timings: `PlanTimingLogger` → `TopAY/src/logs/plan_*.log` (gitignored `**/logs/`). Parallel planner logs **wall-clock** (winning path / barrier), not sum of worker CPUs. Console: `[PlanTiming] ...`.
