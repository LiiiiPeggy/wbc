...
namespace nmoma_planner
{
    ...
    void Planner::init(ros::NodeHandle& nh, const ros::NodeHandle& root_nh)
    {
        ...
        // ################################
        // C++: safeCallback denseness from YAML (减卡)
        // ################################
        nh.param("agent/safe_check_resolution", safe_check_resolution_, 0.05);
        nh.param("agent/safe_check_period", safe_check_period_, 0.10);
        timing_logger_.startSession(moma_param.robot_name);
        int number;
        bool random_ee = true;
                                    ...
                                    const Eigen::VectorXd& start_v)
    {
        ...
        do {
            ...
            topo_select_paths.clear();
            {
                ...
                {
                    finish_timing(false, "moma_parallel");
        ...
        finish_timing(succ, "moma_parallel");
