...
- Consolidated gates (replace deleted `test_topay_*`): `scripts/audit_ranger_geometry.py` (wheels/frames/box STL coverage + outward; `--dump-zero-poses`; `audit_ranger_stl_bounds.py` wraps it); `fake_moma/test_cr10_collision_proxy` (FK/EE + planning transform + colli FD); `map/test_base_obstacle_collision` (GridMap A–D); `planner/test_optimizer_collision_gradient` (`eeCostCallback` base x/y/yaw FD); `planner/test_trajectory_collision_checker` (whole-body traj hard gate A/B/C). Do not reintroduce sphere_visual/cylinder regression binaries — RViz debug only.
