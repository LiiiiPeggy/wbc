...
- Soft ESDF cost alone does **not** guarantee a collision-free published traj. Hard gate: `planner/trajectory_collision_checker` (`checkWholeBodyTrajectoryCollision`) dense-samples via `GridMap::isWholeBodyCollision` (includes `getBaseObstaclePts`). **Sole traj collision authority** on optimize success path: `optimizeTraj` → `printConstraintsSituations` (limits/report only) → `checkWholeBodyTrajectoryCollision` → publish. Do **not** embed the dense hard sweep inside `printConstraintsSituations` (duplicate lag). Run hard gate **only** when `shouldRunWholeBodyTrajHardGate(opt_ok, traj.is_init)` (skip on opt/print fail). `safeCallback` uses YAML `agent/safe_check_resolution` + `safe_check_period` with **`ros::WallTime`** throttle. Return **true = safe**.
...
- Planner traj hard-gate regression: `planner/test_trajectory_collision_checker` Cases A (clear), B (mid box penetration), C (yaw sweep). Rebuild only the test `.o` via `make -f .../build.make` when full `catkin_make` keeps recompiling `grid_map`/`planner`.
- Bridge/arch: `generateBridge` returns **physical** left/right pillar + lintel boxes. Placement AABB footprint is overlap-only (not pushed to `obs_boxes` / `global_boxes`). Default clearance **1.5 m**; opening **~2.8–3.5 m**. Gate: `test_bridge_obstacle_clearance` Case A (2D free), B1 (cargo box vs ~0.30 m lintel), B2 (CR10 arm vs 1.5 m lintel), C (pillar).
- Plan stage timings: `PlanTimingLogger` → `TopAY/src/logs/plan_*.log` using **`ros::WallTime`** for durations (`front_ms` = MCRRT or OMPL; `total_ms` = whole-plan wall latency, not stage sum). Log `front=mcrrt|ompl`. Console: `[PlanTiming] ...`.
...
- Consolidated gates (replace deleted `test_topay_*`): `scripts/audit_ranger_geometry.py` (wheels/frames/box STL coverage + outward; `--dump-zero-poses`; `audit_ranger_stl_bounds.py` wraps it); `fake_moma/test_cr10_collision_proxy` (FK/EE + planning transform + colli FD); `map/test_base_obstacle_collision` (GridMap A–D); `planner/test_optimizer_collision_gradient` (`eeCostCallback` base x/y/yaw FD); `planner/test_trajectory_collision_checker` (whole-body traj hard gate A/B/C); `planner/test_bridge_obstacle_clearance` (bridge opening / lintel / pillar). Do not reintroduce sphere_visual/cylinder regression binaries — RViz debug only.
