...
- Consolidated gates (replace deleted `test_topay_*`): `scripts/audit_ranger_geometry.py` (wheels/frames/box coverage; `audit_ranger_stl_bounds.py` wraps it); `fake_moma/test_cr10_collision_proxy` (FK/EE + planning transform + colli FD); `map/test_base_obstacle_collision` (GridMap A/B); `planner/test_optimizer_collision_gradient` (`eeCostCallback` base x/y/yaw FD). Do not reintroduce sphere_visual/cylinder regression binaries — RViz debug only.
