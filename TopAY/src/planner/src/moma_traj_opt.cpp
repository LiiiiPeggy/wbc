...
namespace nmoma_planner 
{
    ...
    double MomaTrajOpt::eeCostCallback(void* ptrObj, const Eigen::VectorXd& x, Eigen::VectorXd& grad)
    {
        ...
        // ################################
        std::vector<Eigen::Vector4d> base_obstacle_pts = obj.moma_param->getBaseObstaclePts(now_pos);
        ...
        for (size_t bidx = 0; bidx < base_obstacle_pts.size(); ++bidx)
        {
            ...
            {
                obj.smoothL1Penalty(violaPos, violaPosPena, violaPosPenaD);
                double base_colli = mani_colli_weight * violaPosPena;
                grad_to_pos = -mani_colli_weight * violaPosPenaD * grad_pc * cost_scale;
                sdf_cost += base_colli;
            }
        ...
        for (size_t cidx=0; cidx<colli_pts.size(); cidx++)
        {
            const double self_radius_i = obj.moma_param->getColliSelfRadius(cidx);
            const bool check_chassis = (obj.moma_param->kinematics == KinematicsType::Cr10)
                ? !obj.moma_param->isChassisArmCollisionIgnored(obj.moma_param->colli_link_map(cidx))
                : (cidx > 2);
            // with chassis
            if (check_chassis)
            {
                double height = obj.moma_param->chassis_height + self_radius_i - colli_pts[cidx](2);
                if (height > 0)
                {
            ...
            for (size_t cj=cidx+1; cj<colli_pts.size(); cj++)
            {
                ...
                const double self_sum = self_radius_i + obj.moma_param->getColliSelfRadius(cj);
                                    ...
                                    const Eigen::MatrixXd& boundary_acc_)
    {
        ...
        // ################################
        int variable_num = 2 * piece_num
                         + (1 + static_cast<int>(moma_param->dof_num)) * (piece_num - 1);
        Eigen::VectorXd x;
        x.resize(variable_num);
        int opt_var_idx = 0;
                        ...
                        if (check_chassis)
                        {
                            ...
                            if (height > 0)
                            {
                                ...
                                if (debugm)
                                {
                                    ...
                                    PRINT_RED("self_radius_i: "<<self_radius_i);
                                    PRINT_RED("moma_param->chassis_height + self_radius_i: "<<moma_param->chassis_height + self_radius_i);
